...
y_cen = 0
z = np.exp(
    -0.5 * ((((x - x_cen) ** 2 + (y - y_cen) ** 2) ** 0.5 - mean) / var) ** 2
) / (var * (2 * pi) ** 0.5)
z = np.tanh(z)
fig = plt.figure()
...
np.exp(-0.5 * ((((x - x_cen) ** 2 + (y - y_cen) ** 2) ** 0.5 - mean) / var) ** 2) / (
    var * (2 * pi) ** 0.5
)
np.tanh(0)
#%%
...
print(
    ...
    len(poly_coords[0]),
    len(poly_coords[0][0]),
    len(poly_coords[0][0][0]),
    type(poly_coords[0][0][0][0]),
)
