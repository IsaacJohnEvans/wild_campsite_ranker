    ...
    for i in range(len(poly_dict["features"])):
        ...
        for j in range(len(poly_dict["features"][i]["geometry"]["coordinates"])):
            ...
            for k in range(len(poly_dict["features"][i]["geometry"]["coordinates"][j])):
                ...
                poly_point_refs = []
                for l in range(
                    len(poly_dict["features"][i]["geometry"]["coordinates"][j][k])
                ):
                    poly_point.append(
                        (
                            list(
                                reversed(
                                    poly_dict["features"][i]["geometry"]["coordinates"][
                                        j
                                    ][k][l]
                                )
                            )
...
def make_grid(x_cen, y_cen, n_points, min_point, max_point):
    x = np.outer(
        np.linspace(min_point + x_cen, x_cen + max_point, n_points), np.ones(n_points)
    )
    y = np.outer(
        np.linspace(min_point + y_cen, y_cen + max_point, n_points), np.ones(n_points)
    ).T
    z = np.zeros(x.shape)
    return x, y, z
...
def polygon_to_points(x, y, polygon):
    points = np.concatenate(
        (np.reshape(x, (x.size, 1)), np.reshape(y, (y.size, 1))), axis=1
    )
    ...
    for i in range(len(data_list)):
        if "ele" in data_list[i]["properties"]:
            ...
            if data_list[i]["properties"]["class"] not in coords.keys():
                ...
                if (
                    data_list[i]["geometry"]["type"]
                    not in coords[data_list[i]["properties"]["class"]].keys()
                ):
                    coords[data_list[i]["properties"]["class"]][
                        data_list[i]["geometry"]["type"]
                    ] = []
                coords[data_list[i]["properties"]["class"]][
                    data_list[i]["geometry"]["type"]
                ].append(data_list[i]["geometry"]["coordinates"])
...
x, y, z, min_points, max_points = add_feature(
    x, y, z, poly_grid_refs, values, dist, effect, struct, sigma
)
plot_poly(x, y, z)
...
print(
    z[z == np.max(z)].shape,
    np.unravel_index(np.argmax([z == np.max(z)], keepdims=True), z.shape),
)
